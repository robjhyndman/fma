...
<br/>
This can be done using the `ggseasonplot()` function from the `forecast` package:<br/>
```{r fig.width=7.15, fig.height=5}
...
<br/>
**Table 2-7** takes the `beer` data set and shows the **lagged** series, then shows the calculation of **autocorrelation** and **autocovariance** for this lag of one period. The `lag()` function is demonstrated, and the calculations of autocorrelation and autocovariance are shown for a lag of one period:<br/>
<br/>
```{r}
beer
```
...
stats::lag(beer, -2)
```
Autocorrelation and autocovariance are then calculated using **Equation (2.10)** and **Equation (2.11)**, and a lag of one period. The R code below reproduces the calculations in the textbook in **Table 2-7**.
```{r}
beer_lag1 <- stats::lag(beer, -1)

mean <- sum(beer) / length(beer)

autocovar_lag1 <- sum((beer - mean) * (beer_lag1 - mean)) / length(beer)

autocorr_lag1 <- sum((beer - mean) * (beer_lag1 - mean)) / sum((beer - mean)^2)
...
<br/><br/>
```{r}
fcst <- window(beer, start=1995)
fcst_lag1 <- window(stats::lag(beer, -1), start=1995)

accuracy(fcst_lag1, fcst)
```
<br/><br/>
...
<br/>
One possible transformation is the *square root* function. This transform is plotted below: <br/>
```{r fig.width=7.15, fig.height=5}
elec %>%
  sqrt() %>%
  autoplot() +
...
p2 <- elec ^ (1/3) %>%
  autoplot() +
  ggtitle("Cube Root of Electricity Production") +
  xlab("Year") +
  ylab("million kWh ^ 1/3")

# create plot for logarithm
p3 <- elec %>%
  log() %>%
  autoplot() +
  ggtitle("Logarithm of Electricity Production") +
  xlab("Year") +
  ylab("log(million kWh)")

# create plot for negative reciprocal
p4 <- (-1 / elec) %>%
  autoplot() +
  ggtitle("Negative Reciprocal of Electricity Production") +
  xlab("Year") +
  ylab("-1 / million kWh")
...
p2 <- cbind(milk/monthdays(milk)) %>%
  autoplot() +
  ggtitle("Adjusted Monthly Milk Production per Cow") +
  xlab("Months") +
  ylab("Pounds")
